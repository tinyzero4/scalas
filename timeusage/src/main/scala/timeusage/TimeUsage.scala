...
object TimeUsage {
  ...
  def dfSchema(columnNames: List[String]): StructType = {
    val headColumn = StructField(columnNames.head, StringType, nullable = false)
    val tailColumns = columnNames.tail.map(a => StructField(a, DoubleType, nullable = false))
    StructType(headColumn :: tailColumns)
  }
  ...
  def classifiedColumns(columnNames: List[String]): (List[Column], List[Column], List[Column]) = {
    ...
    val primaryActivities = columnNames.filter(c => primary.exists(c.startsWith))
    val workingActivities = columnNames.filter(c => working.exists(c.startsWith))
    val otherActivities = columnNames.filter(c => other.exists(c.startsWith) && !(primaryActivities ::: workingActivities).contains(c) )

    (primaryActivities.map(new Column(_)), workingActivities.map(new Column(_)), otherActivities.map(new Column(_)))
  }
  ...
  def timeUsageSummary(
    ...
    val sexProjection: Column = when($"tesex".equalTo(1), "male").otherwise("female").as("sex")
